...
int16_t control_rpm(int16_t throttle_position, bool singal) {

    if (singal == true) {
        base_rpm = 1000;
    }
    else {
        const int16_t gear_position = 3;  // 3단 고정

        // 스로틀 입력(0~1023 ADC)을 0.0~1.0 비율로 정규화
        throttle_position = saturate(throttle_position, 0, 1023);
        float throttle_ratio = throttle_position / 1023.0f;
        ...
        float engine_torque = throttle_ratio * max_engine_torque;
        ...
        (void)vehicle_speed_kph;

        if (throttle_position == 0) {
            base_rpm -= 20;
        }

        const int16_t idle_rpm = 800;
        if (base_rpm < idle_rpm) base_rpm = idle_rpm;

        const int16_t max_rpm = 6500;
