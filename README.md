...
> ✅ 송신(Node1)과 수신(Node2)이 모두 `[1:2]=throttle / [3:4]=lidar` 순서로 일치합니다.
...
> 임베디드·실시간·보안이 겹치는 환경에서 마주친 문제들과 해결 과정입니다.
> *(아래 `✍️` 표시는 본인이 실제 느낀 점으로 채울 자리입니다.)*

### 🔧 주요 트러블슈팅

<details>
<summary><b>1. 송·수신 노드의 CAN 바이트 매핑 불일치</b></summary>

- **문제:** Node1은 `[1:2]=throttle / [3:4]=lidar` 로 싣는데, Node2는 같은 자리를 `lidar / throttle` 로 해석해 두 센서값이 뒤바뀜
- **원인:** 두 노드가 공유해야 할 **프레임 포맷 명세 없이** 송·수신 코드를 따로 작성
- **해결:** 송신측을 정본으로 삼아 수신 복원 순서를 일치시키고, 프레임 포맷을 README·다이어그램으로 명문화
- 💬 **배운 점:** 이론상 단순해 보이던 CAN 송수신도, 실제로는 **양 끝단의 바이트 약속(프레임 명세)이 정확히 맞아야** 동작한다는 것을 실전에서 체감했다.

</details>

<details>
<summary><b>2. CAN 수신 버퍼의 dangling pointer</b></summary>

- **문제:** `CAN_readMsg()` 가 함수 지역(스택) 변수 `CANFDMessage.data` 의 주소를 반환 → 함수 종료 후 무효 포인터를 참조
- **원인:** 수신 데이터의 **수명(lifetime)** 을 고려하지 않은 포인터 반환
- **해결:** `static` 버퍼에 `memcpy` 로 복사한 뒤 그 주소를 반환
- 💬 **배운 점:** 임베디드 C에서는 포인터가 가리키는 메모리의 **수명(스택 vs 정적)** 을 직접 따져야 한다는 것을 디버깅으로 익혔다.

</details>

<details>
<summary><b>3. 형식적으로만 동작하던 서명 검증</b></summary>

- **문제:** `micro_verify()` 결과를 출력만 하고, 검증에 실패해도 그대로 복호화·제어를 진행 → 보안 검증이 무의미
- **원인:** 검증 반환값을 **제어 흐름에 반영하지 않음**
- **해결:** 검증 실패 시 메시지를 폐기(`TerminateTask`)하도록 분기 추가
- 💬 **배운 점:** '검증을 **수행**한다'와 '검증 결과로 **동작을 차단**한다'는 별개의 문제이며, 보안은 후자까지 이어져야 의미가 있음을 알게 됐다.

</details>

<details>
<summary><b>4. OSEK 구성과 코드의 정합성 (리소스·태스크 종료)</b></summary>

- **문제:** `ResRpm` 이 `conf.oil` 에 선언되지 않은 채 `#define ResRpm 0` 으로 사용(ERIKA `RES_SCHEDULER` 와 충돌 소지) + `ReadCAN` 태스크에 `TerminateTask()` 누락(OSEK 위반)
- **원인:** OIL 구성 파일과 애플리케이션 코드 사이의 정합성 부족
- **해결:** `RESOURCE ResRpm` 정식 선언 + 태스크에 할당, 누락된 `TerminateTask()` 보완
- 💬 **배운 점:** RTOS는 애플리케이션 코드뿐 아니라 **구성 파일(OIL)과의 약속(리소스 선언, 태스크 종료 규칙)** 까지 맞아야 정상 동작한다는 점을 체득했다.

</details>

### 🧗 기술적으로 도전적이었던 점
- **2KB SRAM 제약** 안에서 RTOS + CAN-FD 드라이버 + 암호화를 동시 구동 (힙 여유를 `freeMemory()` 로 추적)
- **8바이트 CAN 프레임** 안에 `암호문 5B + 서명 3B` 를 배치하는 페이로드 설계
- DES·RSA·해시를 **8비트 MCU용 경량 알고리즘**으로 구현
- **Arduino ECU ↔ Jetson(OP-TEE/TrustZone)** 이기종 보안 게이트웨이 연동
- 타이머·버튼 ISR과 **이벤트 기반 OSEK 멀티태스킹** 설계

### 📚 종합 회고

강의에서 **이론으로만 다루던 CAN 통신을 실제 하드웨어(MCP2517FD)로 직접 구현**해보고, **ERIKA RTOS 환경에서 태스크·알람·인터럽트의 타이밍을 직접 맞춰보며** 책으로는 얻기 어려운 실전 경험을 쌓았다. 그 위에 암호화·서명 같은 보안 계층을 8비트 MCU에 얹고, 두 노드(Node1·Node2)로 역할을 나눠 인터페이스를 맞추는 과정에서 **이론과 실제의 간극**을 직접 체감한 프로젝트였다.
...
---
*국민대학교 2025-1학기 「자율주행 컴퓨팅 플랫폼」 Final Project · 학습/연구 목적의 임베디드 차량 보안 프로토타입*
